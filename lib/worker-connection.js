 ...
 * @constructor
 */
function WorkerConnection(connection, worker) {
  /** @type {String} */
...
WorkerConnection.prototype.subscribe = function(channelId, data, msgStartTime) {
  ...
  if (!this._worker.hasChannel(channelId)) {
    this._worker.createChannel(channelId);
  }
  var channel = this._worker.getChannel(channelId);
  clearTimeout(channel.closeTimeout);
  channel.addSubscriber(new Subscriber(this, data));
  _.each(channel.getMessages(), function(msg) {
    if (msg.timestamp > msgStartTime) {
      this.write(msg.content);
    }
  }, this);
  this._worker.sendUpSubscribe(channelId, this.id, data);
};
...
WorkerConnection.prototype.unsubscribe = function(channelId) {
  ...
  var channel = this._worker.getChannel(channelId);
  this._worker.sendUpUnsubscribe(channelId, this.id);

  channel.unsubscribe(this);
  if (!channel.hasSubscribers()) {
    this._worker.delayedCloseChannel(channelId);
  }
};
...
WorkerConnection.prototype.publish = function(channelId, event, data) {
  event = 'client-' + event;
  this._worker.sendUpPublish(channelId, event, data);
};
...
WorkerConnection.prototype.message = function(clientKey, data) {
  this._worker.sendUpMessage(clientKey, data);
};
