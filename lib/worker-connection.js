 ...
 * @param {WorkerUtil} workerUtil
 * @constructor
 */
function WorkerConnection(connection, workerChannels, workerUtil) {
  /** @type {String} */
...
WorkerConnection.prototype.subscribe = function(channelId, data, msgStartTime) {
  ...
  this._workerUtil.sendUpSubscribe(channelId, this.id, data);
};
...
WorkerConnection.prototype.unsubscribe = function(channelId) {
  ...
  var channel = this._workerChannels.get(channelId);
  this._workerUtil.sendUpUnsubscribe(channelId, this.id);
...
WorkerConnection.prototype.publish = function(channelId, event, data) {
  event = 'client-' + event;
  this._workerUtil.sendUpPublish(channelId, event, data);
};
...
WorkerConnection.prototype.message = function(clientKey, data) {
  this._workerUtil.sendUpMessage(clientKey, data);
};
