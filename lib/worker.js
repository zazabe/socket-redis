		...
		sockjsServer.on('connection', function(connection) {
			...
			connection.on('data', function(data) {
				try {
					...
					check(data.event, 'Missing `event`').notEmpty();
					var eventData = data.data;
					switch (data.event) {
						case 'subscribe':
							check(eventData.channel, 'Missing `data.channel`').notEmpty();
							check(eventData.data, 'Missing `data.data`').notEmpty();
							check(eventData.start, 'Missing `data.start`').isInt();
						...
						case 'publish':
							check(eventData.channel, 'Missing `data.channel`').notEmpty();
							check(eventData.data, 'Missing `data.data`').notEmpty();
							check(eventData.event, 'Missing `data.event`').notEmpty();
