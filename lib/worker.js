...
Worker.prototype._listen = function(port) {
  ...
  this._sockjsServer.on('connection', function(connection) {
    ...
    self._addConnection(new WorkerConnection(connection, self._channelList));
  });
...
Worker.prototype.sendDownPublish = function(channelId, event, data) {
  ...
  if (!channel) {
    channel = this._channelList.createChannel(channelId);
...
Worker.prototype._getChannelsData = function() {
  var channelsData = {};
  _.each(this._channelList.getChannels(), function(channel, channelId) {
