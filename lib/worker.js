...
function Worker(port, sockjsClientUrl, sslOptions) {
  ...
  this._connections = [];
  /** @type {WorkerUtil} */
  this._workerUtil = new WorkerUtil(process);
...
Worker.prototype.triggerEventDown = function(type, data) {
  switch (type) {
    ...
    case 'down-status-request':
      this._workerUtil.sendUpStatusRequest(data.requestId, this._getChannelsData());
      break;
    default:
...
Worker.prototype._listen = function(port) {
  ...
  this._sockjsServer.on('connection', function(connection) {
    ...
    self._addConnection(new WorkerConnection(connection, self._channelList, self._workerUtil));
  });
