	...
	var connectRedisDown = function(redisHost) {
		...
		redisClientDown.on("message", function(channel, event) {
			try {
				...
				switch (event.type) {
					case 'publish':
						check(event.data.channel, 'Missing `data.channel`').notEmpty();
						check(event.data.event, 'Missing `data.event`').notEmpty();
						check(event.data.data, 'Missing `data.data`').notEmpty();
